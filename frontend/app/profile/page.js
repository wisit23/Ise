...
const INITIAL_COUPONS = [
  ...
  },
  {
    ...
    description: "ลด 15% สูงสุด 150 บาท สำหรับสินค้าหมวดเสื้อผ้าและรองเท้ามือสอง",
    minSpend: 500,
    discountAmount: "15%",
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    apiFetch("/api/orders/mine?limit=50", { token })
      ...
      .catch((err) =>
        setOrdersError(err.message || "โหลดคำสั่งซื้อไม่สำเร็จ"),
      )
      ...
      <div className="mx-auto w-full max-w-5xl flex-1 px-4 py-8">
          ...
          <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-3 border-t border-line pt-4 sm:border-t-0 sm:pt-0">
              ...
              <div className="flex items-center gap-2 rounded-lg bg-surface-subtle px-3.5 py-2 text-xs text-ink-muted">
                ...
                <span>{coupons.filter((c) => c.status === "active").length} โค้ดส่วนลด</span>
              </div>
            </div>
          </div>
        ...
        {activeTab === "profile" && (
          <section className="rounded-b-lg border border-t-0 border-line bg-white p-6 shadow-1">
            <div className="mb-6">
              <h2 className="text-lg font-bold text-gray-900">ตั้งค่าข้อมูลส่วนตัว</h2>
              <p className="mt-1 text-sm text-ink-muted">
        ...
        {activeTab === "orders" && (
          <section className="rounded-b-lg border border-t-0 border-line bg-white p-6 shadow-1">
            <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <h2 className="text-lg font-bold text-gray-900">สถานะสินค้าและคำสั่งซื้อ</h2>
                <p className="mt-1 text-sm text-ink-muted">
                  ข้อมูลสถานะล่าสุดจากระบบคำสั่งซื้อ
                </p>
              </div>
              <Button href="/orders" variant="secondary" icon="receipt_long" size="sm">
                    ...
                    >
                      ...
                      <div className="flex flex-col gap-4 p-4 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <div className="flex shrink-0 items-center gap-2 border-t border-line pt-3 sm:border-t-0 sm:pt-0">
                          {["pending", "pending_payment"].includes(ord.status) && (
                            <Button
                              href={
        ...
        {activeTab === "address" && (
          <section className="rounded-b-lg border border-t-0 border-line bg-white p-6 shadow-1">
            <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <h2 className="text-lg font-bold text-gray-900">ที่อยู่สำหรับจัดส่ง</h2>
                <p className="mt-1 text-sm text-ink-muted">
                  จัดการที่อยู่สำหรับรับสินค้าของคุณ สามารถตั้งที่อยู่เริ่มต้นได้
                </p>
              </div>
              <Button
                variant="primary"
                  ...
                  >
                    <div>
                      ...
                      <p className="mt-2 text-sm leading-relaxed text-gray-700">
                        {addr.addressLine} แขวง/ตำบล{addr.subdistrict} เขต/อำเภอ{addr.district} จังหวัด{addr.province} {addr.postalCode}
                      </p>
                    </div>
        ...
        {activeTab === "coupons" && (
          <section className="rounded-b-lg border border-t-0 border-line bg-white p-6 shadow-1">
            <div className="mb-6">
              <h2 className="text-lg font-bold text-gray-900">โค้ดส่วนลดของฉัน</h2>
              <p className="mt-1 text-sm text-ink-muted">
                กรอกโค้ดส่วนลดเพื่อเก็บสิทธิ์ หรือเลือกดูคูปองที่คุณมีสำหรับใช้ช้อปปิ้ง
              </p>
            </div>
            ...
            {couponNotice && <Alert tone="success" className="mb-4">{couponNotice}</Alert>}
            {couponError && <Alert tone="error" className="mb-4">{couponError}</Alert>}
            ...
            <div className="mb-6 flex gap-2 border-b border-line pb-3 text-sm font-medium">
              ...
              >
                โค้ดที่ใช้ได้ ({coupons.filter((c) => c.status === "active").length})
              </button>
              <button
                type="button"
              ...
              >
                หมดอายุ / ใช้แล้ว ({coupons.filter((c) => c.status === "expired").length})
              </button>
              <button
                type="button"
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between border-t border-line bg-surface-subtle px-4 py-2.5 text-xs">
                        ...
                        {!isExpired ? (
                          ...
                          <span className="text-xs text-gray-400">หมดอายุแล้ว</span>
                        )}
                      </div>
                    </div>
                  );
