...
function AdminInboxSection({ token }) {
  ...
  useEffect(() => {
    ...
    const params = new URLSearchParams({ page, limit: PAGE_SIZE, scope: "all", status: "ESCALATED" });
    if (q) params.set("q", q);
    
    const pTickets = apiFetch(`/api/support/tickets/queue?${params}`, { token }).catch(() => ({ items: [], totalPages: 1 }));
    
    // Fetch Reports
    const repParams = new URLSearchParams();
    repParams.set("status", statusFilter || "OPEN"); 
    const pReports = apiFetch(`/api/auth/admin/reports?${repParams}`, { token }).catch(() => ({ items: [], totalPages: 1 }));
  ...
  async function handleReportAction(e) {
    ...
    if (!reportDecision) { setActionError("กรุณาเลือกการตัดสินใจ"); return; }
    if (!reportReason.trim()) { setActionError("กรุณาระบุเหตุผล"); return; }
  ...
  async function handleBanUser(userId) {
    if (!window.confirm("คุณแน่ใจหรือไม่ที่จะระงับบัญชีผู้ใช้นี้? (SUSPEND_USER)")) return;
            ...
            {items.map((t) => (
              <tr key={t.id} onClick={() => setSelectedTicket(t)} className="group hover:bg-slate-50/80 hover:shadow-sm transition-all duration-200 cursor-pointer">
                ...
                <td className="px-5 py-4">
                  {t._type === "REPORT" ? (
                    <Badge text="ด่วนที่สุด (CRITICAL)" style="bg-red-100 text-red-700 border border-red-200" />
                  ) : (
                    <Badge
                ...
                <div className="bg-white border border-red-100 rounded-xl p-6 shadow-sm mb-6 relative overflow-hidden">
                  ...
                  <div className="space-y-3">
                    <div className="flex gap-4">
                      <span className="w-24 text-sm font-medium text-slate-500">ผู้รายงาน:</span>
                      <span className="text-sm font-mono">{selectedTicket.rawReport.reporterId}</span>
                    </div>
                    {selectedTicket.rawReport.targetId && (
                      <div className="flex gap-4">
                        <span className="w-24 text-sm font-medium text-slate-500">ผู้ใช้เป้าหมาย:</span>
                        <span className="text-sm font-mono text-red-600">{selectedTicket.rawReport.targetId}</span>
                      </div>
                    )}
                    {selectedTicket.rawReport.productId && (
                      <div className="flex gap-4">
                        <span className="w-24 text-sm font-medium text-slate-500">สินค้าเป้าหมาย:</span>
                        <span className="text-sm font-mono text-red-600">{selectedTicket.rawReport.productId}</span>
                      </div>
                    )}
                    <div className="flex gap-4 pt-3 border-t border-slate-100 mt-3">
                      <span className="w-24 text-sm font-medium text-slate-500">รายละเอียด:</span>
                      <span className="text-sm font-medium text-slate-800">{selectedTicket.rawReport.reason}</span>
                    </div>
                  </div>
                </div>

                {selectedTicket.rawReport.status !== "ACTIONED" && selectedTicket.rawReport.status !== "DISMISSED" ? (
                  <form onSubmit={handleReportAction} className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
                    <h4 className="font-bold text-slate-900 mb-4">พิจารณาและจัดการ</h4>
                    
                    <div className="mb-4">
                      <label className="block text-sm font-medium text-slate-700 mb-1.5">การตัดสินใจ</label>
                      <select 
                    ...
                    <div className="mb-4">
                      <label className="block text-sm font-medium text-slate-700 mb-1.5">หมายเหตุ (ภายใน / ส่งให้ผู้ใช้)</label>
                      <textarea 
                        value={reportReason}
