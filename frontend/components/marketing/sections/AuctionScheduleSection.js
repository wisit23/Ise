...
const ROUND_PHASE_STYLE = {
  ...
  auction: "bg-emerald-100 text-emerald-800 border-emerald-300 font-bold animate-pulse",
...
export function RoundManagementSection({ token, onRoundCreated }) {
  ...
  function loadRounds() {
    setLoading(true);
    Promise.all([
      apiFetch("/api/products/auctions/rounds/current", { token }).catch(() => null),
      apiFetch("/api/products/auctions/rounds", { token }).catch(() => ({ items: [] })),
    ])
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 pb-4 border-b border-slate-100">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            กำหนดช่วงเวลารับสินค้าและช่วงเวลาเริ่มประมูลจริงสำหรับผู้ขาย พร้อมป้องกันรอบซ้อนทับ
          </p>
        </div>
        <button
        ...
        <form onSubmit={handleCreateRound} className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50/50 p-4 animate-fade-in-up">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
            ...
            <div className="space-y-3 rounded-lg bg-white p-3 border border-emerald-100">
              ...
              <div>
                <label htmlFor="subStartsAt" className="block text-slate-500 mb-0.5">วัน-เวลาเริ่มเปิดรับ</label>
                <input
                  id="subStartsAt"
                  required
                  type="datetime-local"
              ...
              <div>
                <label htmlFor="subEndsAt" className="block text-slate-500 mb-0.5">วัน-เวลาปิดรับสินค้า</label>
                <input
                  id="subEndsAt"
                  required
                  type="datetime-local"
            ...
            <div className="space-y-3 rounded-lg bg-white p-3 border border-emerald-100">
              ...
              <div>
                <label htmlFor="aucStartsAt" className="block text-slate-500 mb-0.5">วัน-เวลาเริ่มเปิดประมูล</label>
                <input
                  id="aucStartsAt"
                  required
                  type="datetime-local"
              ...
              <div>
                <label htmlFor="aucEndsAt" className="block text-slate-500 mb-0.5">วัน-เวลาสิ้นสุดการประมูล</label>
                <input
                  id="aucEndsAt"
                  required
                  type="datetime-local"
      ...
      <div className="mt-4">
        {loading ? (
                ...
                {phase && (
                  <span
                    className={`rounded-full border px-2.5 py-0.5 text-[11px] font-semibold ${
                      ROUND_PHASE_STYLE[phase] || "bg-slate-100 text-slate-600 border-slate-200"
                    }`}
                  >
          ...
          <div className="rounded-xl border border-dashed border-slate-200 p-4 text-center text-xs text-slate-500">
            ยังไม่มีรอบการประมูลที่กำลังดำเนินอยู่หรือกำลังจะมาถึง กรุณากด &ldquo;สร้างรอบประมูลใหม่&rdquo; ด้านบนเพื่อกำหนดช่วงเวลารับสินค้า
          </div>
        )}
      </div>
              ...
              <tbody className="divide-y divide-slate-100">
                {allRounds.map((r) => (
                  <tr key={r.id} className="hover:bg-slate-50/50">
                    <td className="py-2.5 font-semibold text-slate-900">{r.title}</td>
                    <td className="py-2.5">
                      <span
                        className={`inline-block rounded-full border px-2 py-0.5 text-[10px] font-medium ${
                          ROUND_PHASE_STYLE[r.phase] || "bg-slate-100 text-slate-600 border-slate-200"
                        }`}
                      >
                    ...
                    <td className="py-2.5 text-slate-600">
                      {new Date(r.submissionStartsAt).toLocaleString("th-TH")} — {new Date(r.submissionEndsAt).toLocaleString("th-TH")}
                    </td>
                    <td className="py-2.5 text-slate-600">
                      {new Date(r.auctionStartsAt).toLocaleString("th-TH")} — {new Date(r.auctionEndsAt).toLocaleString("th-TH")}
                    </td>
