    ...
    <div className="animate-fade-in-up space-y-6">
      ...
      <div className="flex flex-col gap-3 rounded-xl border border-slate-200/70 bg-white p-4 shadow-sm sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-2 text-slate-800">
          ...
          <span className="text-sm font-semibold">ช่วงเวลาการวัดผล (Attribution Window)</span>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
        ...
        <KpiCard
          ...
          value={loading ? "…" : `฿${(metrics.grossRevenue || 0).toLocaleString()}`}
          icon="payments"
          color="sky"
          sub="ก่อนหักส่วนลด"
        />
        <KpiCard
          label="ส่วนลดทั้งหมด"
          value={loading ? "…" : `฿${(metrics.totalDiscount || 0).toLocaleString()}`}
          icon="loyalty"
          color="amber"
        ...
        <KpiCard
          ...
          value={loading ? "…" : `฿${(metrics.netRevenue || 0).toLocaleString()}`}
        ...
        <KpiCard
          ...
          value={loading ? "…" : `${metrics.totalClaimed || 0} / ${metrics.totalRedeemed || 0}`}
        ...
        {loading ? (
          ...
          <div className="py-12 text-center">
            ...
            <p className="mt-1 text-xs text-slate-400">
              เมื่อมีการสั่งซื้อที่ใช้คูปองเสร็จสมบูรณ์ ตัวเลขยอดขายและการแปลงสิทธิ์จะปรากฏที่นี่
            </p>
          </div>
        ) : (
            ...
            <table className="w-full text-left text-xs text-slate-600">
              <thead className="border-b border-slate-200/80 bg-slate-50/70 text-[11px] font-semibold uppercase text-slate-500">
                <tr>
                  ...
                  <th className="px-3 py-2.5 text-center">เก็บคูปอง (Claimed)</th>
                  <th className="px-3 py-2.5 text-center">ใช้สำเร็จ (Redeemed)</th>
                  ...
                  <th className="px-3 py-2.5 text-center font-bold text-violet-700">Conversion %</th>
                </tr>
              </thead>
                ...
                {campaigns.map((c) => (
                  <tr key={c.campaignId} className="hover:bg-slate-50/60 transition-colors">
                    <td className="px-3 py-3">
                      <div className="font-bold text-slate-900">{c.campaignCode || c.campaignId}</div>
                      {c.campaignName && (
                        <div className="text-[11px] text-slate-500">{c.campaignName}</div>
                      )}
                    </td>
