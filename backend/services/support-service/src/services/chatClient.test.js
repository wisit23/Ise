...
test("createSupportConversation sends greeting on 201 (new room)", async () => {
  ...
  global.fetch = (url, options) => {
    ...
    if (url.includes("/internal/conversations")) {
      return Promise.resolve({
        ok: true,
        status: 201,
        json: () => Promise.resolve({ id: "conv-new-1", contextType: "SUPPORT" }),
      });
    }
  ...
  try {
    const conv = await chatClient.createSupportConversation("t-1", "#CS-000001", "u-1");
  ...
  global.fetch = (url, options) => {
    ...
    if (url.includes("/internal/conversations")) {
      ...
      return Promise.resolve({
        ok: true,
        status: 200,
        json: () => Promise.resolve({ id: "conv-existing-1", contextType: "SUPPORT" }),
      });
    }
  ...
  global.fetch = (url) => {
    if (url.includes("/status")) {
      return Promise.resolve({
        ok: false,
        status: 500,
        json: () => Promise.resolve({ error: "Failed to update status in MongoDB" }),
      });
    }
    return Promise.reject(new Error(`Unexpected url: ${url}`));
  };
...
test("requestWithTimeout extracts actionable error and retains HTTP status", async () => {
  ...
  global.fetch = () => {
    return Promise.resolve({
      ok: false,
      status: 409,
      json: () => Promise.resolve({ message: "Conversation is already locked" }),
    });
  };
