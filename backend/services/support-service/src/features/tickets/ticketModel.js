  ...
  try {
    return await prisma.$transaction(async (tx) => {
      ...
      if (isCustomerFacingAgentReply) {
        await (
          typeof tx.supportTicket.updateMany === "function"
            ? tx.supportTicket.updateMany.bind(tx.supportTicket)
            : tx.supportTicket.update.bind(tx.supportTicket)
        )({
          where: {
            id: ticket.id,
