...
const buyerToken = signAccessToken({ sub: "buyer-123", role: "BUYER" });
const strangerToken = signAccessToken({ sub: "stranger-999", role: "BUYER" });
const agentAToken = signAccessToken({ sub: "agent-A", role: "CUSTOMER_SERVICE" });
const agentBToken = signAccessToken({ sub: "agent-B", role: "CUSTOMER_SERVICE" });
const adminToken = signAccessToken({ sub: "admin-1", role: "ADMIN" });
const safetyToken = signAccessToken({ sub: "safety-1", role: "TRUST_AND_SAFETY" });
...
test("Internal chat events endpoint security and validation", async (t) => {
  ...
  await t.test("rejects request with invalid internal token with 403", async () => {
    const res = await request(app)
      .post("/internal/chat-events")
  ...
  await t.test("rejects request with missing chatMessageId with 400", async () => {
    const res = await request(app)
      .post("/internal/chat-events")
...
test("ticketModel.recordChatMessage idempotency, correlation and firstResponseAt", async (t) => {
  ...
  prisma.ticketMessage.findUnique = async ({ where }) => {
    return storedMessages.find((m) => m.chatMessageId === where.chatMessageId) || null;
  };
  ...
  await t.test("rejects correlation mismatch between ticket and conversation", async () => {
  ...
  await t.test("first delivery creates exactly one TicketMessage and one REPLY audit log", async () => {
    ...
    assert.ok(fakeTicket.firstResponseAt, "firstResponseAt should be updated for agent reply");
  });

  await t.test("second delivery with same chatMessageId is idempotent (no duplicate message or audit)", async () => {
    ...
    assert.equal(storedMessages.length, 1, "Must still have exactly one TicketMessage");
    assert.equal(storedAudits.length, 1, "Must still have exactly one audit log");
  });

  await t.test("requester message does not update firstResponseAt", async () => {
    ...
    assert.equal(fakeTicket.firstResponseAt, null, "firstResponseAt must NOT be set by requester message");
  });
});
...
test("Ticket Chat Join/Continue Authorization, Same-Room Continuation and Privacy", async (t) => {
  ...
  chatClient.createSupportConversation = async (id, number, reqId) => {
  ...
  await t.test("normal CS agent cannot access or join an ESCALATED ticket", async () => {
    const res = await request(app)
  ...
  await t.test("normal CS agent cannot join a ticket assigned to another agent", async () => {
    const res = await request(app)
  ...
  await t.test("ADMIN can continue an ESCALATED ticket in the existing room without replacing it", async () => {
    addedParticipants = [];
    auditedActions = [];
    ...
    assert.equal(res.body.conversationId, sharedConversationId, "Must return the SAME conversationId");
    assert.equal(res.body.role, "ADMIN", "Admin must have ADMIN per-room role");
  ...
  await t.test("TRUST_AND_SAFETY can continue an ESCALATED ticket", async () => {
    const res = await request(app)
      .post(`/tickets/${escalatedTicketId}/continue`)
  ...
  await t.test("CS agent can join an unassigned ticket and missing conversation is repaired idempotently", async () => {
  ...
  await t.test("internal notes are never exposed in requester-facing ticket view", async () => {
    ...
    assert.equal(ticket.messages.some((m) => m.isInternal), false);
    assert.equal(ticket.messages.some((m) => m.body.includes("Private")), false);
  });
});
