...
process.env.DATABASE_URL ||= process.env.DATABASE_URL_PRODUCT || "postgresql://reloop:reloop_dev_password@localhost:5432/reloop_product";
if (process.env.DATABASE_URL.includes("@postgres:")) {
  process.env.DATABASE_URL = process.env.DATABASE_URL.replace("@postgres:", "@localhost:");
}
const REDIS_URL = (process.env.REDIS_URL || "redis://localhost:6379").replace("@redis:", "@localhost:");
...
test("Auction Core, Concurrency, Soft Close & BullMQ Integration Suite", async (t) => {
  ...
  t.after(async () => {
    ...
    try {
      const bidWhere = [];
      if (createdBidIds.length > 0) bidWhere.push({ id: { in: createdBidIds } });
      if (createdAuctionIds.length > 0) bidWhere.push({ auctionId: { in: createdAuctionIds } });
    ...
    try {
      const auctionWhere = [];
      if (createdAuctionIds.length > 0) auctionWhere.push({ id: { in: createdAuctionIds } });
      if (createdRoundIds.length > 0) auctionWhere.push({ roundId: { in: createdRoundIds } });
      if (createdProductIds.length > 0) auctionWhere.push({ productId: { in: createdProductIds } });
  ...
  async function createTestProduct(sellerId, title = "Auction Item Test Product", status = "available") {
  ...
  async function createTestRound({ subStartsInMs = -3600000, subEndsInMs = 3600000, aucStartsInMs = 3600000, aucEndsInMs = 7200000 } = {}) {
    const now = Date.now();
  ...
  await t.test("Step 1: Seller submits product to active round -> persists in PostgreSQL with status 'auction'", async () => {
    ...
    assert.equal(res.status, 201, `Expected 201 Created: ${JSON.stringify(res.body)}`);
    ...
    assert.equal(freshProduct.status, "auction", "Product status must be 'auction' to isolate from regular feed");
  });

  await t.test("Step 2: Admin Decoupling & Marketing Approval -> direct to 'scheduled' and BullMQ booked", async () => {
    const round = await createTestRound();
    const product = await createTestProduct("seller-auction-test-01");
    ...
    assert.equal(adminRes.status, 403, "Admin must not be allowed to approve auctions");
    ...
    const inDb = await prisma.auctionItem.findUnique({ where: { id: auctionId } });
  ...
  await t.test("Step 3: Bidding rules & PostgreSQL persistence", async () => {
    ...
    assert.equal(lowBidRes.status, 400, "Bid below startingPrice must be rejected");
    ...
    assert.equal(bid1Res.status, 201, `Expected 201: ${JSON.stringify(bid1Res.body)}`);
    ...
    assert.equal(underIncRes.status, 400, "Bid must meet highest + bidIncrement");
  ...
  await t.test("Step 4: Idempotency Key DB constraint prevents duplicate bids on retry", async () => {
    const round = await createTestRound();
    const product = await createTestProduct("seller-auction-test-01");
    ...
    assert.equal(secondAttempt.status, 201, "Idempotent retry must return 201 with existing bid");
    ...
    const bidRows = await prisma.bid.findMany({ where: { idempotencyKey: idemKey } });
    assert.equal(bidRows.length, 1, "Must not create duplicate bid in PostgreSQL");
    ...
    assert.equal(diffAmountRes.status, 409, "Reusing key with different amount must return 409 Conflict");
    ...
    const diffBidderRes = await request(app)
      ...
      .set("Authorization", `Bearer ${buyer2Token}`)
      .send({ amount: 150, idempotencyKey: idemKey });
    assert.equal(diffBidderRes.status, 409, "Reusing key with different bidder must return 409 Conflict");
    ...
    const diffAuctionRes = await request(app)
      ...
      .set("Authorization", `Bearer ${buyer1Token}`)
      .send({ amount: 150, idempotencyKey: idemKey });
    assert.equal(diffAuctionRes.status, 409, "Reusing key with different auction must return 409 Conflict");
    ...
    const retryAfterClosedRes = await request(app)
      ...
      .set("Authorization", `Bearer ${buyer1Token}`)
      .send({ amount: 150, idempotencyKey: idemKey });
    assert.equal(retryAfterClosedRes.status, 201, "Exact retry on closed auction must return original bid");
    assert.equal(retryAfterClosedRes.body.id, firstAttempt.body.id);
  });

  await t.test("Step 5: Concurrent bidding serialized by pg_advisory_xact_lock", async () => {
    const round = await createTestRound();
    const product = await createTestProduct("seller-auction-test-01");
    ...
    assert.deepEqual(statuses, [201, 400], "Concurrent same-amount bids must result in exactly 1 winner and 1 rejection");
  ...
  await t.test("Step 6: Anti-Sniping Soft Close extends scheduledEndAt by +5m in PostgreSQL", async () => {
    const round = await createTestRound();
    const product = await createTestProduct("seller-auction-test-01");
    ...
    const updatedAuction = await prisma.auctionItem.findUnique({ where: { id: auctionId } });
    const expectedEndTime = new Date(initialEndTime.getTime() + 5 * 60 * 1000);

    const diffMs = Math.abs(updatedAuction.scheduledEndAt.getTime() - expectedEndTime.getTime());
    assert.ok(diffMs < 1000, `scheduledEndAt should be extended by +5m (diff: ${diffMs}ms)`);

    // Verify BullMQ job delay/timestamp corresponds to updated scheduledEndAt
    const rescheduledJob = await auctionCloseQueue.getQueue().getJob(auctionId);
    assert.ok(rescheduledJob, "Rescheduled close job must exist in BullMQ queue");
    assert.equal(rescheduledJob.id, auctionId);
    assert.ok(rescheduledJob.opts.delay > 0, "Rescheduled job must have a positive delay");
    const jobExecutionTime = rescheduledJob.timestamp + rescheduledJob.opts.delay;
    const timeDiffMs = Math.abs(jobExecutionTime - updatedAuction.scheduledEndAt.getTime());
    assert.ok(
      timeDiffMs < 2000,
  ...
  await t.test("Step 7: Auction Close with Winner -> calls Order Client once and records winningOrderId", async (t) => {
    const round = await createTestRound();
    const product = await createTestProduct("seller-auction-test-01");
    ...
    assert.equal(orderClientCallCount, 1, "Order Client must be called exactly once");
  ...
  await t.test("Step 8: Real BullMQ delayed worker execution & idempotent re-close prevents duplicate orders", async (t) => {
    const round = await createTestRound();
    const product = await createTestProduct("seller-auction-test-01");
    ...
    assert.ok(scheduledJob, "BullMQ delayed close job must exist before worker runs");
    ...
    while (Date.now() - pollStart < 5000) {
      const item = await prisma.auctionItem.findUnique({ where: { id: auctionId } });
    ...
    assert.ok(closedDbItem, "Auction must be closed by BullMQ worker within timeout");
    ...
    assert.equal(orderClientCalls, 1, "BullMQ worker must invoke Order Client exactly once");
    assert.equal(orderClientPayload?.auctionId, auctionId);
    assert.equal(orderClientPayload?.buyerId, "buyer-auction-test-01");
    ...
    assert.equal(orderClientCalls, 1, "Idempotent re-close must not invoke order client again");
  ...
  await t.test("Step 9: No-bid Auction close reverts product status to 'available'", async () => {
    const round = await createTestRound();
    const product = await createTestProduct("seller-auction-test-01");
    ...
    const freshProduct = await prisma.product.findUnique({ where: { id: product.id } });
    assert.equal(freshProduct.status, "available", "No-bid auction close must return product to available");
  });

  await t.test("Step 10: Auction Round Overlap Protection, Concurrency & Deterministic Selection", async () => {
    // Determine safe future range after max existing auctionEndsAt to avoid colliding with seed data
    const maxRound = await prisma.auctionRound.aggregate({ _max: { auctionEndsAt: true } });
    const maxEndMs = maxRound._max.auctionEndsAt ? maxRound._max.auctionEndsAt.getTime() : 0;
    ...
    assert.equal(resA.status, 201, `Failed to create first round: ${JSON.stringify(resA.body)}`);
    ...
    assert.ok(
      resOverlap.body.error?.includes(roundATitle) || resOverlap.body.message?.includes(roundATitle),
    ...
    assert.equal(resB.status, 201, `Back-to-back round must succeed: ${JSON.stringify(resB.body)}`);
    ...
    const currentSub = await auctionService.getCurrentRound(new Date(testBaseMs + 1 * HOUR_MS));
    ...
    const currentWaiting = await auctionService.getCurrentRound(new Date(testBaseMs + 2.5 * HOUR_MS));
    ...
    const currentAuc = await auctionService.getCurrentRound(new Date(testBaseMs + 4 * HOUR_MS));
    ...
    const currentUpcoming = await auctionService.getCurrentRound(new Date(testBaseMs - 1 * HOUR_MS));
    ...
    const currentEnded = await auctionService.getCurrentRound(new Date(testBaseMs + 100 * 24 * HOUR_MS));
    ...
    const activeSub = await auctionRepository.findActiveSubmissionRound(new Date(testBaseMs + 1 * HOUR_MS));
    assert.equal(activeSub?.id, resA.body.id, "Repository must select the only active submission round (Step 1 verifies live POST /auctions round attachment)");
