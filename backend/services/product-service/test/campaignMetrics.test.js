...
const { createCampaignMetricsService } = require("../src/features/campaigns/campaignMetrics");
...
test("Campaign Metrics, Attribution & Conversion Suite", async (t) => {
  ...
  await t.test("recordOrderCompletedEvent: ignores orders without campaign", async () => {
  ...
  await t.test("recordOrderCompletedEvent: records attribution and deduplicates retries (idempotency)", async () => {
    const event = {
      eventId: "evt-attr-1",
      orderId: "ord-attr-1",
  ...
  await t.test("getCampaignMetrics: calculates conversion rate (redeemedCount / claimedCount * 100)", async () => {
    ...
    const metrics = await metricsService.getCampaignMetrics({
      campaign: { id: "camp-1", code: "CAMP1", name: "Campaign One", usedCount: 10 },
      campaignId: "camp-1",
    });
  ...
  await t.test("getCampaignMetrics: returns 0 values for campaign with no orders (no error)", async () => {
    const metrics = await metricsService.getCampaignMetrics({
      campaign: { id: "camp-empty", code: "EMPTY", name: "Empty Camp", usedCount: 5 },
  ...
  await t.test("getOverviewMetrics: aggregates revenue and orders across all campaigns", async () => {
    await metricsService.recordOrderCompletedEvent({
      eventId: "evt-ov-1",
      orderId: "ord-ov-1",
      campaignId: "camp-1",
  ...
  await t.test("getSalesTrends: groups metrics by daily series", async () => {
    ...
    const trends = await metricsService.getSalesTrends({ campaignId: "camp-1" });
