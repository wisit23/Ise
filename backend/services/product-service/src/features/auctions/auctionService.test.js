...
test("placeBid throws 409 Conflict when idempotencyKey is reused with different amount", async (t) => {
  ...
  await assert.rejects(
    ...
    (err) => err.status === 409 && err.message.includes("idempotency key reused"),
  );
});
...
test("createRound validates input dates and boundaries", async () => {
  ...
  await assert.rejects(
    ...
    (err) => err.status === 400 && err.message.includes("submissionEndsAt must be after submissionStartsAt"),
  );
  ...
  await assert.rejects(
    ...
    (err) => err.status === 400 && err.message.includes("auctionStartsAt must be after or equal to submissionEndsAt"),
  );
  ...
  await assert.rejects(
    ...
    (err) => err.status === 400 && err.message.includes("auctionEndsAt must be after auctionStartsAt"),
  );
});
...
test("createRound throws 409 Conflict when overlapping round exists", async (t) => {
  ...
  t.mock.method(repository, "withRoundLock", async (fn) => fn("mock-tx"));
  t.mock.method(repository, "findConflictingRound", async ({ subStart, aucEnd }, tx) => {
  ...
  await assert.rejects(
    ...
    (err) => {
      ...
      assert.ok(err.message.includes("Round interval overlaps with existing round"));
      return true;
    },
  );
});
