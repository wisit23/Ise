...
const { validateSegmentRule, matchesSegment } = require("./segmentRule");

test("Buyer Segmentation Rule Engine Suite", async (t) => {
  await t.test(
    "validateSegmentRule: accepts empty or null targetSegment",
    () => {
    ...
    assert.throws(
      () =>
        validateSegmentRule({
          type: "rules",
          rules: [
            { field: "favoriteCategory", operator: "like", value: "Shirt" },
          ],
        }),
  ...
  await t.test(
    "matchesSegment: supports preferredSize and sizePreference aliases",
    () => {
      const rule = {
        type: "rules",
  ...
  await t.test(
    "matchesSegment: in and nin operators evaluate correctly",
    () => {
      const inRule = {
        type: "rules",
        rules: [
          { field: "styleTag", operator: "in", value: ["Streetwear", "Y2K"] },
        ],
      };
      assert.equal(
        matchesSegment({ stylePreference: "streetwear" }, inRule),
        true,
      );
      assert.equal(
        matchesSegment({ stylePreference: "Minimal" }, inRule),
        false,
      );
