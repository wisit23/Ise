...
test("TSR-02: Single-owner dispute assignment, unassigned read-only, validation, reassign & escalate", async (t) => {
  ...
  // Production policy deliberately separates staff and customer identities;
  // verify the queue with a valid Customer Service account.
  const agentQueue = await request(app)
    .get("/disputes/queue")
    .set("Authorization", `Bearer ${agent1Token}`);
  assert.equal(agentQueue.status, 200);
