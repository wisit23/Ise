...
} = require("../src/services/holdBackfillService");

async function run() {
  const dryRun = process.argv.includes("--dry-run");
  console.log(
    `[TSR-02] Starting hold backfill and ambiguity audit (dryRun: ${dryRun})...`,
  );
  ...
  try {
    ...
    console.log(
      `  - Ambiguous orders detected: ${result.ambiguousOrdersCount}`,
    );
