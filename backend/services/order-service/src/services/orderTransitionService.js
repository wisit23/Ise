...
async function releaseHold(
  tx,
  { orderId, source, referenceId, reason, releasedBy },
) {
  const where = {
    orderId,
    source,
...
async function resolveHoldState(tx, orderId, { releasingTsHold = false } = {}) {
  ...
  const tsHoldStillOpen =
    !releasingTsHold && order.paymentSimulationStatus === "ON_HOLD";

  // payoutHeld is true if any active OrderHold exists OR legacy flags are still holding
  const isPayoutHeld =
    activeHoldCount > 0 || csCaseStillOpen || tsHoldStillOpen;
