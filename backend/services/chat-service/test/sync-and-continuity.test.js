...
test("supportSyncWorker exponential backoff and bounded lifecycle", async (t) => {
  await t.test("calculateBackoffMs produces exponential backoff with upper bound", () => {
  ...
  await t.test("startSupportSyncWorker and stopSupportSyncWorker are bounded and cleanly stop", () => {
...
test("Chat-to-ticket sync handles outages without losing persisted messages", async (t) => {
  ...
  await t.test("service outage records retry backoff without losing message", async () => {
    global.fetch = async () => {
    ...
    assert.equal(fakeMessage.body, "Can you help with my order?", "Message body preserved");
  });
...
test("Internal notes privacy and query boundaries", async (t) => {
  ...
  await t.test("buildPageQuery allows internal notes when includeInternal is true", () => {
    const query = buildPageQuery({
      conversationId: "conv-1",
      limit: 20,
  ...
  await t.test("broadcastMessage does not emit internal notes to public conversation room", () => {
    let emittedRooms = [];
    const fakeIo = {
      to: (room) => {
