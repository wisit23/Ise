...
| Seller        | `SellerProfile` / `seller_profiles`         | Same fields as ER (shop_name, id_card_number, bank_account) **plus** `address`, `kyc_status`, `kyc_document_url`, `rejection_reason`, `submitted_at`, `reviewed_by`, `verified_at` — required by workflow WF-01 (KYC verification & seller onboarding flow, reviewed by Admin). |
